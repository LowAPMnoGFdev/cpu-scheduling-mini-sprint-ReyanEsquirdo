from collections import deque

class Process:
	def __init__(self, pid, arrival_time, burst_time):
		self.pid = pid
		self.arrival_time = arrival_time
		self.burst_time = burst_time
		self.remaining_time = burst_time
		self.start_time = None
		self.finish_time = None
		self.response_time = None
		self.waiting_time = 0
		self.turnaround_time = 0

def round_robin(processes, quantum):
	n = len(processes)
	time = 0
	ready_queue = deque()
	gantt = []
	completed = 0
	arrived = set()
	proc_map = {p.pid: p for p in processes}

	while completed < n:
		
		for p in processes:
			if p.arrival_time <= time and p.pid not in arrived:
				ready_queue.append(p)
				arrived.add(p.pid)

		if ready_queue:
			proc = ready_queue.popleft()
			start = time
			if proc.start_time is None:
				proc.start_time = time
				proc.response_time = proc.start_time - proc.arrival_time
			run_time = min(quantum, proc.remaining_time)
			time += run_time
			proc.remaining_time -= run_time
			gantt.append(f"[{proc.pid}:{start}–{time}]")
			
			for p in processes:
				if p.arrival_time > start and p.arrival_time <= time and p.pid not in arrived:
					ready_queue.append(p)
					arrived.add(p.pid)
			if proc.remaining_time == 0:
				proc.finish_time = time
				completed += 1
			else:
				ready_queue.append(proc)
		else:
			
			next_arrival = min([p.arrival_time for p in processes if p.pid not in arrived], default=None)
			if next_arrival is not None:
				time = next_arrival

	# Calculate metrics
	for p in processes:
		p.turnaround_time = p.finish_time - p.arrival_time
		p.waiting_time = p.turnaround_time - p.burst_time

	return gantt, processes

def print_metrics(gantt, processes):
	print("Gantt:", "".join(gantt))
	print("Per-process (W / T / R):")
	w_sum = t_sum = r_sum = 0
	for p in processes:
		print(f"{p.pid}: {p.waiting_time} / {p.turnaround_time} / {p.response_time}")
		w_sum += p.waiting_time
		t_sum += p.turnaround_time
		r_sum += p.response_time
	n = len(processes)
	print(f"Averages: Waiting {w_sum/n:.1f}, Turnaround {t_sum/n:.1f}, Response {r_sum/n:.1f}")

if __name__ == "__main__":
	processes = [
		Process("P1", 0, 7),
		Process("P2", 2, 4),
		Process("P3", 4, 1),
		Process("P4", 5, 4),
		Process("P5", 7, 3),
	]
	quantum = 2
	gantt, procs = round_robin(processes, quantum)
	print_metrics(gantt, procs)
